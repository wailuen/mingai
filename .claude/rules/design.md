...
```css
/* Page title */     22px / 700 / Plus Jakarta Sans   → text-page-title
/* Section heading */15px / 600 / Plus Jakarta Sans   → text-section-heading
/* Body / rows */    13px / 400-500 / Plus Jakarta Sans → text-body-default
/* Labels / nav */   11px / 500-600 / Plus Jakarta Sans — letter-spacing .06-.08em; text-transform: uppercase → text-label-nav
/* Data values */    13px / 400-500 / DM Mono (pair with font-mono) → text-data-value
/* Tab bars */       12px / 500 / Plus Jakarta Sans   → text-[12px] (intentional exception)
/* Badges */         10-11px (compact badge exception) → text-[10px] or text-[11px]
```

**CRITICAL**: `text-sm` in Tailwind = 14px. This is NOT in the design system scale. Using `text-sm` for body text collapses the 22–15–13–11 hierarchy by inserting an undifferentiated 14px band. Always use `text-body-default` (13px) for body text, inputs, buttons, labels, and error messages. The ONLY 14px that appears in the system is `text-section-heading` minus 1px — it does not belong in UI body copy.
