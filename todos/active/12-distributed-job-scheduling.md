...
- [x] `CancelledError` propagates cleanly
- [ ] Phase 3 addendum (SCHED-025 dependency): after initial conversion, add `check_missed_job(job_name)` call before the first `await asyncio.sleep()` in the scheduler loop. This will be added when SCHED-025 is implemented.
...
`AgentHealthMonitor` uses a class-based `.start()` method that internally creates its own asyncio task. This is the third scheduling pattern that exists alongside APScheduler (Phase 2 removes APScheduler) and the asyncio loop pattern. After this item, only one canonical pattern remains.

Convert the class to a standalone `run_agent_health_scheduler()` async function using the same `while True / await asyncio.sleep` pattern as all other schedulers. Wrap with `distributed_job_lock("agent_health", ttl_seconds=7200)`. TTL is 7200s (2 hours) — the existing interval is 3600s (1 hour).

`main.py` already has:
```python
monitor = AgentHealthMonitor(db_session_factory=async_session_factory, interval_seconds=3600)
...
- [ ] CI pipeline has a pre-deploy step that checks `CONFIG GET appendonly` on the target Redis instance
- [ ] Step exits with non-zero status (fails the deploy) if result is `no`
- [ ] Step passes if result is `yes`
- [ ] Deployment documentation updated to note the AOF persistence requirement and CI enforcement
- [ ] The CI step is clearly labeled "Redis AOF persistence check — required for distributed job lock correctness"
...
- [ ] `two concurrent coroutines, only one acquires`: run two tasks simultaneously, assert exactly one gets `acquired=True`
- [ ] `second coroutine acquires after TTL expiry`: set short TTL (2s), wait 3s, assert second coroutine acquires
- [ ] `heartbeat prevents TTL expiry`: set TTL=4s, heartbeat_interval=2s, hold lock for 6s, assert lock still held at 6s
- [ ] `release clears key`: after context exits, key no longer exists in Redis
- [ ] `skipped lock does not leave key`: when lock is not acquired, no new key is written
- [ ] All 5 test cases pass against real Redis with `pytest src/backend/tests/integration/test_distributed_lock.py`
- [ ] No mocking of Redis client — uses real Redis connection from test environment
...
- [ ] Test imports `main` module and verifies `asyncio.create_task` is called with a coroutine from `run_tool_health_scheduler`
- [ ] Test fails if `run_tool_health_scheduler` is removed from the lifespan without also removing this test
- [ ] Alternative acceptable approach: run the full lifespan startup with mocked DB/Redis and assert `tool_health_job_started` log event is emitted
- [ ] Test passes with `pytest src/backend/tests/integration/test_tool_health_integration.py`
...
- [ ] (1) Insert 3 `status='running'` rows older than 1 hour → run startup cleanup → assert all 3 rows have `status='abandoned'`
- [ ] (2) Insert 1 `status='running'` row 30 minutes old → run startup cleanup → assert row still has `status='running'` (not yet stale, threshold is 1 hour)
- [ ] (3) Insert rows with statuses `completed`, `failed`, `abandoned` → run startup cleanup → assert all are untouched (only `running` rows are updated)
...
- [ ] `src/backend/app/core/scheduler/tenant_throttle.py` exists
- [ ] File exports a semaphore or throttle primitive keyed on `SCHEDULER_MAX_CONCURRENT_TENANTS` env var (default 5)
- [ ] `query_warming.py` imports and applies the semaphore to its per-tenant iteration loop — at most `SCHEDULER_MAX_CONCURRENT_TENANTS` tenants run concurrently
- [ ] Unit test: 10 tenants, semaphore=3, assert max 3 run concurrently at any given moment
