...
- [ ] Endpoint calls all 3 test prompts against draft profile models
- [ ] Correct token counts and latency in response
- [ ] `estimated_cost_usd` calculated using pricing from `llm_library.pricing_per_1k_tokens_*`
- [ ] Draft profile (not yet Published) testable via this endpoint
- [ ] Frontend slide-in panel shows test results with DM Mono for token/latency/cost values
- [ ] Latency shown in ms; cost in USD with 6 decimal places
- [ ] `require_platform_admin` enforced
- [ ] Timeout: 30s (LLM calls can be slow); 504 if exceeded
...
- [ ] PATCH status=deprecated succeeds; existing assignments preserved
- [ ] `GET /platform/llm-profiles/{id}/tenant-assignments` returns list of tenant IDs/names
- [ ] Deprecated profiles excluded from `GET /admin/llm-config` available profiles list (tenant admin view)
- [ ] Deprecated profiles still returned in `GET /platform/llm-library` (platform admin view — with deprecated status badge)
- [ ] Confirmation dialog in frontend shows tenant count before deprecation
...
- [ ] Tenant drilldown slide-in panel includes "LLM Config" section showing current profile
- [ ] Profile shown as display_name (not raw model name)
- [ ] Platform admin can see if tenant is on BYOLLM or Library mode
- [ ] Best practices notes snippet (first 80 chars) shown in tooltip on profile name
- [ ] 0 TypeScript errors
...
- [ ] Alembic migration file created (next version)
- [ ] UNIQUE constraint on `(tenant_id, date)`
- [ ] All score columns use NUMERIC(5,2) — range 0-100 with 2 decimal places
- [ ] `at_risk_reason` TEXT nullable — human-readable explanation of why at_risk_flag is set
- [ ] RLS: only `platform_admin` scope can access
...
- [ ] All 4 component scores calculated from real data sources
- [ ] Weighted composite = `(usage*0.30) + (breadth*0.20) + (satisfaction*0.35) + (error*0.15)`
- [ ] At-risk detection logic covers all 3 conditions (composite < 40, sat < 50 for 2w, declining 3w)
- [ ] `at_risk_reason` populated with specific trigger ("composite_low", "satisfaction_declining", "usage_trending_down")
- [ ] Upsert pattern: `INSERT ... ON CONFLICT (tenant_id, date) DO UPDATE`
- [ ] Job runtime < 5 minutes for 100 active tenants
- [ ] Job failure for one tenant does not abort others (per-tenant try/except)
- [ ] Unit test: score calculation formula verified for edge cases (0 usage, 100% satisfaction)
...
- [ ] Returns only tenants with `at_risk_flag=true` in most recent snapshot
- [ ] `weeks_at_risk` calculated from consecutive at_risk_flag=true rows ordered by date DESC
- [ ] `component_breakdown` returns all 4 component scores
- [ ] Response sorted by `composite_score ASC` (worst first)
- [ ] Empty array returned (not 404) when no at-risk tenants
- [ ] `require_platform_admin` enforced
...
- [ ] 12 weekly data points returned in `trend` array (ISO week format)
- [ ] Missing weeks (no data) represented as null values (not omitted)
- [ ] All 4 component scores in both current and trend data
- [ ] Composite score consistent with PA-007 weighted formula
- [ ] Response time < 500ms
...
- [ ] Table wired to real API data (no static mocks)
- [ ] Color coding matches design system health thresholds
- [ ] Sparkline shows 12-week composite trend (miniature line chart)
- [ ] At-risk badge visible for tenants with `at_risk_flag=true`
- [ ] Row click navigates to tenant drilldown with health section open
...
- [ ] In-app notification created in `notifications` table with `from_platform_admin=true` flag
- [ ] Email sent via SendGrid if `"email"` in `send_via` list
- [ ] Outreach logged to `audit_log` (actor=platform_admin, target=tenant_id, action="proactive_outreach")
- [ ] `require_platform_admin` enforced
- [ ] 422 if both `subject` and `body` are blank
- [ ] Frontend: "Message Tenant" button in at-risk tenant drilldown panel with compose modal
...
- [ ] `cost_summary_daily` table Alembic migration included
- [ ] Nightly batch job populates `cost_summary_daily` from `usage_events`
- [ ] Upsert on `(tenant_id, date)` conflict
- [ ] `model_breakdown` JSONB structure: `[{ "model": "...", "provider": "...", "tokens_in": N, "tokens_out": N, "cost_usd": X }]`
- [ ] Job runtime < 10 minutes for 100 tenants × 30 days backfill
- [ ] Unit test: aggregation formula verified against known `usage_events` fixture
...
- [ ] `gross_margin_pct` column in `cost_summary_daily`
- [ ] Plan revenue loaded from env: `PLAN_REVENUE_STARTER_DAILY_USD`, `PLAN_REVENUE_PRO_DAILY_USD`, `PLAN_REVENUE_ENTERPRISE_DAILY_USD`
- [ ] Infra cost loaded from env: `INFRA_COST_PER_TENANT_DAILY_USD`
- [ ] Margin capped at 100% and floored at -100% in DB (reality check)
- [ ] Returned in cost API response as `gross_margin_pct`
- [ ] Frontend: margin % displayed with `--accent` if > 60%, `--warn` if 30-60%, `--alert` if < 30%
...
- [ ] Azure Cost Management API queried daily (scheduled job)
- [ ] Credentials from env vars — never hardcoded
- [ ] 24-48h data delay handled: query for `TODAY - 2 days` (always available)
- [ ] Cost data labeled with `last_updated_at` and `is_estimated: false` in API response (vs env constant fallback `is_estimated: true`)
- [ ] Attribution model: total Azure spend divided by active tenant count (proportional by usage)
- [ ] Fallback to env constant if Azure API unavailable (do not crash batch job)
- [ ] Integration test with Azure API mock (real Azure calls in CI would incur cost)
...
- [ ] Per-tenant and global cost alert thresholds configurable
- [ ] Stored in `tenant_configs` under `cost_alerts` key
- [ ] Nightly check: if daily_spend > threshold → create P2 issue + notification
- [ ] Nightly check: if margin_floor < threshold → create P2 issue + notification
- [ ] Duplicate suppression: max 1 alert per tenant per calendar day per alert type
- [ ] `require_platform_admin` on both routes
- [ ] Frontend: cost alert config in tenant drilldown "Finance" section
