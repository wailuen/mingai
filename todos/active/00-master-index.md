...
**Phase 2+ total**: 167 new work items (177 including 8 gated and 2 blocked) across 7 todo files (created 2026-03-15; 5 now in completed/, 2 still active: 02-phase3-auth-flexibility.md and 05-tenant-admin-phase-b-d.md)
...
| `todos/active/05-tenant-admin-phase-b-d.md`      | SSO wiring, access control, agent workspace, mobile              | 36 (TA-001–036)     | 35/36 COMPLETED (all except TA-025 BLOCKED — product gate; 5-10 persona interviews required). TA-001–005/033/035 completed 2026-03-17.                                 |
| `todos/completed/06-agent-registry.md`           | HAR Phase 0-3: registry catalog, A2A gaps, KYB, blockchain       | 17 (HAR-001–017)    | ✅ 12/12 Phase 0-1 COMPLETED — moved to completed/ 2026-03-17. HAR-013–017 GATED (external: 100+ transactions / blockchain deployment). No actionable work remains.    |
...
| `todos/completed/09-dynamic-auth0-entra-sso.md`  | Dynamic Auth0 Azure Entra (waad) connection creation per tenant  | 11 (ENTRA-001–011)  | ✅ ALL 11 COMPLETED — moved to completed/ 2026-03-17. sso_entra.py POST/PATCH configure + test endpoints, lifecycle in workspace.py, router registration, 22 unit tests, useSSO.ts hooks, SSO wizard Entra tab, TypeScript clean (0 errors), API reference docs updated, no new env vars. |
...
| Item       | Description                             | File                                         | Evidence                                                                                                                                                                             |
| ---------- | --------------------------------------- | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| P3AUTH-002 | Auth0 JWKS validation                   | `todos/active/02-phase3-auth-flexibility.md` | `app/modules/auth/jwt.py` — decode_jwt_token_auth0(), \_JWKS_CACHE 3600s TTL, key rotation auto-invalidation. 25 unit + 5 integration tests.                                         |
| P3AUTH-003 | Tenant SSO configuration API            | `todos/active/02-phase3-auth-flexibility.md` | GET/PATCH `/admin/sso/config` in workspace.py. provider_type allowlist. Audit log on PATCH. 20 unit tests.                                                                           |
| P3AUTH-004 | SAML 2.0 SSO wizard API                 | `todos/active/02-phase3-auth-flexibility.md` | `app/modules/admin/sso_saml.py` — configure/sp-metadata/test endpoints. defusedxml + lxml. 22 unit tests.                                                                            |
| P3AUTH-005 | OIDC SSO wizard API                     | `todos/active/02-phase3-auth-flexibility.md` | `app/modules/admin/sso_oidc.py` — OIDC discovery, vault-ref client secret. 14 unit tests.                                                                                            |
| P3AUTH-006 | Google Workspace SSO                    | `todos/active/02-phase3-auth-flexibility.md` | `app/modules/admin/sso_oidc.py` — google-oauth2 strategy. Client secret encrypted.                                                                                                   |
| P3AUTH-007 | Okta SSO                                | `todos/active/02-phase3-auth-flexibility.md` | `app/modules/admin/sso_oidc.py` — OIDC discovery reuse for Okta domain.                                                                                                              |
| P3AUTH-008 | JIT provisioning on first SSO login     | `todos/active/02-phase3-auth-flexibility.md` | `app/modules/auth/jit_provisioning.py` — POST /internal/users/jit-provision. hmac.compare_digest. scripts/auth0/post_login_action.js. 7 unit tests.                                  |
| P3AUTH-009 | Group-to-role sync                      | `todos/active/02-phase3-auth-flexibility.md` | `app/modules/auth/jit_provisioning.py` — POST /internal/users/sync-roles. Highest-privilege role wins. Audit log on change. 2511 unit tests passing.                                 |
| P3AUTH-011 | Session management                      | `todos/active/02-phase3-auth-flexibility.md` | `app/modules/auth/routes.py` — Redis sorted set sessions, plan limits (starter=3/pro=10/enterprise=unlimited), force-logout endpoint.                                                |
| P3AUTH-012 | Migration tooling                       | `todos/active/02-phase3-auth-flexibility.md` | `scripts/migrate_users_to_auth0.py` — idempotent CLI, dry-run, CSV report, 100ms rate limit.                                                                                         |
| P3AUTH-013 | Local auth fallback                     | `todos/active/02-phase3-auth-flexibility.md` | `app/core/dependencies.py` routes HS256 → decode_jwt_token_v1_compat(). Integration test confirms local login with AUTH0_DOMAIN present.                                             |
| P3AUTH-014 | SSO configuration PATCH route wired     | `todos/active/02-phase3-auth-flexibility.md` | `src/web/lib/hooks/useSSO.ts` — 7 new hooks. SSOSetupWizard.tsx fully wired. SP metadata download. 0 TS errors.                                                                      |
| P3AUTH-016 | Auth0 JWKS integration tests            | `todos/active/02-phase3-auth-flexibility.md` | `tests/integration/test_auth0_jwks.py` — 5 tests, graceful skip when AUTH0_DOMAIN absent.                                                                                            |
| P3AUTH-017 | JWT v1/v2 dual acceptance int. tests    | `todos/active/02-phase3-auth-flexibility.md` | `tests/integration/test_jwt_dual_acceptance.py` — 5 tests: v1/v2 HS256+RS256, structure validation, expiry rejection.                                                                |
| P3AUTH-018 | SAML SSO unit tests                     | `todos/active/02-phase3-auth-flexibility.md` | 22 tests in `tests/unit/test_saml_wizard.py` — delivered as part of P3AUTH-004.                                                                                                      |
| P3AUTH-019 | OIDC SSO unit tests                     | `todos/active/02-phase3-auth-flexibility.md` | 14 tests in `tests/unit/test_oidc_wizard.py` — delivered as part of P3AUTH-005/006/007.                                                                                              |
| P3AUTH-020 | JIT provisioning unit tests             | `todos/active/02-phase3-auth-flexibility.md` | `tests/unit/test_jit_provisioning.py` — 7 tests (P3AUTH-008) expanded to 14 with group sync (P3AUTH-009).                                                                            |
| P3AUTH-021 | Auth0 Management API client creds       | `todos/active/02-phase3-auth-flexibility.md` | `app/modules/auth/management_api.py` — get_management_api_token() Redis cache TTL=expires_in-60. 17 unit tests. AUTH0_MANAGEMENT_API_TOKEN deprecated.                               |
| TA-001     | SAML wizard backend API                 | `todos/active/05-tenant-admin-phase-b-d.md`  | P3AUTH-004 endpoints in sso_saml.py under /admin/sso/saml/ with require_tenant_admin. SP metadata entityID verified.                                                                 |
| TA-002     | OIDC wizard backend API                 | `todos/active/05-tenant-admin-phase-b-d.md`  | P3AUTH-005 endpoints in sso_oidc.py under /admin/sso/oidc/ with require_tenant_admin. Auth0 authorize URL on test.                                                                   |
| TA-003     | JIT provisioning                        | `todos/active/05-tenant-admin-phase-b-d.md`  | GET /admin/sso/config returns jit_provisioning block. PATCH accepts jit_default_role. 6 unit tests (TestSSOConfigJITExtension).                                                      |
| TA-004     | Group-to-role mapping UI                | `todos/active/05-tenant-admin-phase-b-d.md`  | `src/web/app/settings/sso/elements/GroupRoleMappingTable.tsx` — Add/Delete rows, role dropdown, Save wired to useUpdateGroupSyncConfig(). 0 TS errors.                               |
| TA-005     | SSO enable/disable toggle               | `todos/active/05-tenant-admin-phase-b-d.md`  | `src/web/app/settings/sso/elements/SSOToggle.tsx` — 3 render states, confirm dialog on disable. 0 TS errors.                                                                         |
| TA-033     | User import from SSO                    | `todos/active/05-tenant-admin-phase-b-d.md`  | `app/modules/admin/sso_import.py` — directory/users + import-from-sso. Migration v037 users.auth0_user_id. 26 unit tests.                                                            |
| TA-035     | Tenant admin role delegation            | `todos/active/05-tenant-admin-phase-b-d.md`  | `app/modules/admin/role_delegation.py` — POST/DELETE/GET delegation. Migration v038 user_delegations + RLS. 21 unit tests.                                                           |
| DEF-008    | Auth0 group sync DB writes + login hook | `todos/active/07-deferred-phase1.md`         | ✅ COMPLETE — `sync_roles_on_login()` calls `sync_team_memberships_db()` on every login. Upsert pattern. `test_group_sync.py` + `test_jit_provisioning.py`. 2529 unit tests passing. |
| DEF-014    | SAML/OIDC SSO integration tests         | `todos/active/07-deferred-phase1.md`         | ✅ COMPLETE — `tests/integration/test_sso_wizards.py` — 6/6 passing (TEST-026/027). Real PostgreSQL, Auth0 Management API mocked. `_run_sync()` KQueue fix.                          |
