...
| ---------------------------------------------- | ------------------------------------------------------------------------------ | -------------------- | -------------------------- |
| `todos/active/12-distributed-job-scheduling.md` | Distributed Redis-lock job scheduling — replace APScheduler, wrap all 13 jobs | 38 (SCHED-001–038)   | ACTIVE — all pending 2026-03-20 |

**Plan reference**: `workspaces/mingai/02-plans/13-distributed-job-scheduling-plan.md` (v1.1)
**Red team**: `workspaces/mingai/01-analysis/01-research/48-distributed-job-scheduling-redteam.md` (2 CRITICAL · 7 HIGH · 4 MEDIUM · 2 LOW)

**Phase breakdown**:
- Phase 1 (Foundation): SCHED-001–007 — `DistributedJobLock`, `seconds_until_utc()`, v042 migration, zombie cleanup, APScheduler shutdown — must ship before any second pod is added
- Phase 2 (Conversion): SCHED-008–022 — convert all 13 jobs, fix H-01/H-02/H-05/H-06/H-07 bugs, remove APScheduler
- Phase 3 (Observability): SCHED-023–028 — `job_run_context()`, missed-job detection, Platform Admin history endpoint, Tenant Admin sync-status, job history UI
- Phase 4 (Hardening): SCHED-029–030, SCHED-037–038 — Redis AOF CI pipeline check, Kubernetes grace period, startup AOF warning, per-tenant query_warming throttle
- Testing (all phases): SCHED-031–036

**Dependencies**: None — Phase 1 is self-contained. v042 migration requires current alembic head = 041.

**Pre-existing bugs fixed by this work**:
- H-07: `tool_health_scheduler` has never run in any environment (not wired in `main.py`) → fixed in SCHED-008
- H-01: Tool health failure counter uses `==` threshold check (race condition with multiple pods) → fixed in SCHED-008
- H-04: APScheduler `.shutdown()` never called in lifespan teardown → fixed in SCHED-007
