...
- [ ] Key schema matches exactly: `mingai:{tenant_id}:intent:{sha256(normalized_query)}`
- [ ] Query normalization function in `app/core/cache_utils.py` (shared with CACHE-002)
- [ ] Cache check happens before LLM call in `intent_detector.py`
- [ ] Cache write is async fire-and-forget (non-blocking)
- [ ] TTL: 86400s confirmed via `redis.ttl(key)` in tests
- [ ] Unit test: same query normalized to same key from different whitespace/case variants
- [ ] Integration test (real Redis): cache hit avoids LLM call (mock LLM call counter)
- [ ] Cross-tenant isolation: `tenant_id` in key prevents cross-tenant cache hits
...
- [ ] Key schema includes `model_id` (deployment name from `llm_library` or env var)
- [ ] Embedding serialized as float16 binary bytes (not JSON array)
- [ ] Deserialized back to float32 list before use (precision acceptable for similarity search)
- [ ] Storage size reduction: float16 binary < 50% of JSON float32 representation (verified in unit test)
- [ ] TTL: 604800s
- [ ] Cross-tenant isolation enforced via key
- [ ] Unit test: serialize → store → retrieve → deserialize roundtrip produces vectors within epsilon=0.001
...
- [ ] Key schema as specified (including truncated hashes)
- [ ] Version tag stored alongside result: `{ "version": N, "results": [...] }`
- [ ] Version validation on cache hit: if stored version != current INCR counter, treat as miss
- [ ] TTL defaults to 3600s; overridden by CACHE-005 per-index config
- [ ] Cache write is async fire-and-forget
- [ ] Integration test (real Redis): cache hit returns stored results; version mismatch forces re-search
...
- [ ] `INCR mingai:{tenant_id}:version:{index_id}` called on every document sync event (add/update/delete)
- [ ] Counter initializes to 0 on first INCR (Redis INCR behavior — no explicit initialization needed)
- [ ] Counter value read in CACHE-003 via `GET mingai:{tenant_id}:version:{index_id}`
- [ ] No TTL on version counter (persists indefinitely; reset only on explicit index deletion)
- [ ] Integration test: document update increments counter; subsequent search cache check uses new version
...
- [ ] `PATCH /admin/indexes/{id}/cache-config` validates TTL against allowed set
- [ ] Plan tier enforcement: 403 if Starter/Professional selects > 3600s
- [ ] `GET /admin/indexes/{id}/cache-config` returns current TTL (or default 3600 if not set)
- [ ] Config stored in `tenant_configs` JSON
- [ ] CACHE-003 reads per-index TTL at cache write time
- [ ] Frontend dropdown shows only plan-appropriate options (greyed-out vs 403)
- [ ] 0 TypeScript errors in frontend component
...
- [ ] Scheduled cron trigger at 03:00 UTC (initial version; tenant timezone in future iteration)
- [ ] Reads top-100 queries from `profile_learning_events` ordered by frequency DESC, last 30 days
- [ ] Rate limiting: `asyncio.sleep(0.1)` between embedding calls (10/sec max)
- [ ] Skip if `mingai:{tenant_id}:emb:{model_id}:{hash}` already exists in Redis
- [ ] Job processes one tenant at a time (sequential, not concurrent)
- [ ] Logs: tenant_id, queries warmed, queries skipped, total time, errors
- [ ] Integration test (real Redis): verify embeddings present after warm-up run
...
- [ ] `lookup()` uses pgvector `<=>` cosine operator with parameterized threshold
- [ ] `lookup()` filters by `tenant_id` AND `expires_at > NOW()` AND `version_tag = current`
- [ ] `store()` is async non-blocking (fire-and-forget via `asyncio.create_task`)
- [ ] Stored `response_json` contains only `CacheableResponse` fields (no user-specific data)
- [ ] `invalidate_version()` increments stored version_tags for tenant (or targeted index_id)
- [ ] Unit test: lookup returns None below threshold; returns match above threshold
- [ ] Integration test (real pgvector): end-to-end lookup → store → hit roundtrip
...
- [ ] Both dataclasses defined with all listed fields
- [ ] `CacheableResponse` is JSON-serializable via `model_dump()` (Pydantic BaseModel)
- [ ] `PersonalizedResponse` wraps `CacheableResponse` (composition, not inheritance)
- [ ] Chat pipeline updated to construct `PersonalizedResponse` from either cache hit or fresh pipeline output
- [ ] Semantic cache stores `CacheableResponse.model_dump_json()` in `response_json` column
- [ ] Unit test: serialization/deserialization roundtrip preserves all fields
...
- [ ] Semantic cache check occurs after query embedding, before intent detection
- [ ] Cache hit bypasses entire pipeline (intent, search, LLM call)
- [ ] SSE stream on cache hit: sends `cache_state` event (CACHE-012) then response events
- [ ] SSE stream on cache miss: sends `cache_state` miss event then normal pipeline events
- [ ] Cache store on miss: async non-blocking, does NOT delay SSE response
- [ ] If `SemanticCacheService.lookup()` raises exception: log + continue as cache miss (no 500)
- [ ] Integration test: cache hit response returns in < 200ms (vs 2-5s pipeline)
...
- [ ] Sync worker calls `CACHE-004` INCR after each document sync
- [ ] Sync worker calls `CACHE-008.invalidate_version(tenant_id, index_id)` after each sync
- [ ] Both calls are non-blocking (async fire-and-forget)
- [ ] Integration test: document update triggers both invalidation paths; subsequent cache lookup returns miss
...
- [ ] `cache_state` SSE event emitted before any `content` events
- [ ] `hit: true` with `similarity` and `age_seconds` on cache hit
- [ ] `hit: false` with `stage: "semantic"` on cache miss
- [ ] Frontend `CacheStateChip.tsx` receives and displays the event without changes
- [ ] `age_seconds` calculated from `created_at` to `NOW()` in UTC
- [ ] Integration test: SSE stream includes `cache_state` event as first event type
...
- [ ] Threshold validation: 0.85 <= threshold <= 0.99; 422 outside range
- [ ] TTL validation: 3600 <= ttl_seconds <= 604800; 422 outside range
- [ ] Config stored in `tenant_configs` under `semantic_cache_config` key
- [ ] `SemanticCacheService.lookup()` reads threshold from `tenant_configs` (or default 0.92 if not set)
- [ ] `require_tenant_admin` enforced
- [ ] GET returns defaults if not configured (not 404)
...
- [ ] Job runs hourly (cron or APScheduler)
- [ ] Expired entries deleted: `DELETE FROM semantic_cache WHERE tenant_id = $1 AND expires_at < NOW()`
- [ ] Stale version entries detected via join against current version counters (Redis)
- [ ] Job logs cleanup counts per tenant
- [ ] Integration test: insert expired row → run job → verify row deleted
- [ ] Job failure does NOT crash the application (wrapped in try/except with logging)
...
- [ ] Cache events written to analytics table on every hit and miss
- [ ] `cost_saved_usd` calculated from avoided LLM call cost (use P2LLM-011 cost model)
- [ ] Event emission is async non-blocking (does not delay SSE response)
- [ ] All 4 event types emitted at correct points (hit, miss, eviction in cleanup job, invalidation on doc update)
- [ ] Integration test (real PostgreSQL): verify event rows present after cache hit and miss
...
- [ ] `?period=7d|30d|90d` query param on all endpoints
- [ ] `top-cached-queries` returns SHA256 prefix only (privacy — no raw user queries)
- [ ] Hit rate = `cache_hits / (cache_hits + cache_misses)` per period
- [ ] Response time < 1s for 30-day queries (aggregation index-backed)
- [ ] `require_tenant_admin` enforced
...
- [ ] All static/mock data replaced with real API calls
- [ ] Period selector wired (refetch on change)
- [ ] Loading skeleton during fetch
- [ ] Empty state when no cache events in period
- [ ] DM Mono for all numeric values
- [ ] 0 TypeScript errors
- [ ] No purple/blue colors; use `--accent` for hit rate, `--warn` for miss rate
...
- [ ] `X-Cache-Bypass: true` header detected in chat route → skips `SemanticCacheService.lookup()`
- [ ] Cache age (seconds since `created_at`) returned in `cache_state` SSE event as `age_seconds`
- [ ] `CacheStateChip.tsx` displays age in human-readable format ("2h ago", "5m ago")
- [ ] [Refresh] button in chip sends new request with bypass header
- [ ] Bypass request still stores result back to cache (update cache with fresh data)
- [ ] 0 TypeScript errors in `CacheStateChip.tsx` modifications
...
- [ ] All 6 scenarios with real Redis and PostgreSQL/pgvector
- [ ] Cross-tenant isolation test inserts cache entry for tenant A, queries as tenant B — expects miss
- [ ] Threshold boundary test uses known embedding vectors with calculated cosine similarity
- [ ] All tests pass: `pytest tests/integration/test_cache_integration.py`
