...
- [x] ProfileForm shows 4 model slots with provider + deployment name per slot
- [x] Markdown editor for `best_practices_md` with preview pane
- [x] Draft→Publish action: validates all 4 slots have non-empty deployment names
- [x] Publish→Deprecate action: shows confirmation dialog listing tenants currently on this profile
- [x] Deprecated profiles shown with `--text-faint` styling (not red — deprecated is not an error)
- [x] 0 TypeScript errors
- [x] Matches Obsidian Intelligence design: no purple, DM Mono for model names
...
- [x] Profile change PATCH triggers immediate Redis DEL for tenant config cache key
- [x] Subsequent LLM calls (within 60s) use new profile (verified by model name in `usage_events`)
- [x] Integration test: profile change → verify new model reflected in usage_events within 2 subsequent calls
- [x] No restart required for config change propagation
...
- [x] User PII: `name` → "DELETED*USER*{uuid*prefix}", `email` → "deleted*{uuid}@gdpr.invalid"
