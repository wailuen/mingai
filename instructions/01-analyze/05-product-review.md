...
Make sure the below are captured

1. Platform:
a. for configuration of LLM, need to setup model for the following:
i. intent 
ii. main chat
iii. vision model - extract from picture
iv. agent model 

2. Tenant :
a. need to configure their own sharepoint connection, sync schedule and folder. Check aihub2 codebase for sharepoint configuration and sync-worker implementation
b. extend to support google drive for similar like above
c. manage tenant level glossary
d ...

3. End user :
a. can upload document by dragging into browser or paste it. see aihub2 for how this is implemented

4. For RBAC, what about platform level role

5. Fixes all issues from Red Team Findings

6. Document in details, your work in `workspaces/<project-directory>/01-analysis/01-research`.
