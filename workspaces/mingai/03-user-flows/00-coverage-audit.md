...
| 1. Tenant Lifecycle | Plan upgrade/downgrade                              | —                 | **MISSING FLOW** |
| 1. Tenant Lifecycle | Invoice/billing admin                               | —                 | **MISSING FLOW** |
...
| 4. LLM Config       | Profile version update → tenant notification        | —                 | **MISSING FLOW** |
| 4. LLM Config       | LLM profile test before activating                  | —                 | **MISSING FLOW** |
| 5. Cost/Token       | Cross-tenant cost monitoring                        | 11, 05            | COVERED          |
| 5. Cost/Token       | Per-tenant cost drill-down                          | 02 (F6), 11       | COVERED          |
| 5. Cost/Token       | Chargeback to billing                               | —                 | PARTIAL          |
| 6. Agent Templates  | Template library management                         | 11                | COVERED          |
| 6. Agent Templates  | Tenant deploys template                             | 12 (F7)           | COVERED          |
| 6. Agent Templates  | Template versioning (v2 publish + tenant migration) | —                 | **MISSING FLOW** |
| 6. Agent Templates  | Platform admin QA before publish                    | —                 | **MISSING FLOW** |
| 7. Tool Catalog     | Tool registration                                   | 11                | COVERED          |
| 7. Tool Catalog     | Tenant admin browses + enables tools                | —                 | **MISSING FLOW** |
...
| 1. Workspace Setup      | MFA enforcement for non-SSO users         | —                | **MISSING FLOW** |
| 1. Workspace Setup      | Workspace branding (ongoing, post-wizard) | —                | PARTIAL          |
...
| 2. Identity + Doc Store | Document-level permission verification    | —                | **MISSING FLOW** |
...
| 4. Glossary             | Multilingual query expansion              | 15 (partial)     | PARTIAL          |
...
| 5. RBAC                 | Agent cloning/duplication                 | —                | **MISSING FLOW** |
...
| 6. Agent Workspace      | Agent version update (live agent)         | —                | **MISSING FLOW** |
...
| Conversation search                   | —            | **MISSING FLOW**          |
| Conversation sharing                  | —            | PARTIAL (noted as future) |
...
## Missing Flows — Not Yet Added (Future Work)

These gaps exist but require product decisions before flows can be written:

| Gap                                              | Blocking Decision                                                     |
| ------------------------------------------------ | --------------------------------------------------------------------- |
| LLM profile version update → tenant notification | Product: how are tenants notified? Force upgrade or choose?           |
| Agent template versioning (v2 publish)           | Product: semver policy for templates? Automatic migration?            |
| Platform admin QA before publishing template     | Product: what QA checklist is required?                               |
| Conversation sharing                             | Product: current scope is link-only; full collab deferred to Phase 5+ |
| Conversation search                              | Product: full-text vs semantic search? Backend indexing strategy?     |
| Tool catalog tenant browsing                     | Product: is this a separate UI section or part of agent config?       |
| Agent cloning/duplication                        | Product: copy-on-write or reference?                                  |
| Document-level permission verification           | Product: how to surface doc-level RBAC results to admin?              |
| MFA enforcement flow                             | Product: per-user enforce or global policy? Grace period?             |
| Billing plan upgrade/downgrade                   | Product: Stripe integration, proration policy                         |
| Agent live version update                        | Product: blue/green deploy? Conversation migration?                   |
...
**Flows added**: SSO group→RBAC, agent discovery, 3-rating escalation, plan upgrade
