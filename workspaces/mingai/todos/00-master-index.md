...
**Last updated**: 2026-03-23 (TODO-39 and TODO-40 COMPLETE — Tool Catalog API alignment (retire + health endpoints) and Tool Catalog 3-section UI redesign per product spec. v061 migration (tool_health_checks table), tool_health_job.py extended (INSERT per check + 30-day cleanup), POST /platform/tool-catalog/{id}/retire (soft-delete, audit log), GET /platform/tool-catalog/{id}/health (history endpoint), hard-delete 409 guard (active assignments), list query extended (executor_type, scope, source_mcp_server_id, is_active). Frontend: Tool interface extended, classifyTools() helper, IntegrationGroupRow.tsx (new, collapsible MCP group rows), ToolList.tsx rewritten to 3-section layout with retire confirmation modal, ToolRegistrationForm.tsx hint added. 0 TypeScript errors. active/ dir now empty.) Prior: 2026-03-22 (ALL ACTIVE TODOS COMPLETE — 3246 unit tests passing, 0 failures. Completed: LLM-QG-5 security + code review agents round-2 validation (7 findings resolved: api_key min_length=8, LATERAL dedup join, cache invalidation after slot assignment, \_SLOT_COL module-level constant, showBYOLLM useEffect sync, aws/bedrock provider map, fe80::/10 SSRF range). Also completed: TODO-26 Bedrock provider (BEDROCK-001–021), TODO-27 LLM Profile schema v050 migration, TODO-28 SSRF middleware, TODO-29 LLMProfileService, TODO-30 ProfileResolver (3-tier caching), TODO-31 InstrumentedClient updates, TODO-32 Platform Profile API, TODO-33 Tenant BYOLLM API, TODO-34 backend tests, TODO-38 health monitoring background job. Also completed: TODO-13–25 Agent Studio (Tool Catalog, Skills Library, Agent Templates, MCP integration, A2A protocol). active/ directory is empty. Prior: TODO-15/BEDROCK EXPANDED to 21 items (BEDROCK-001–021) + merged to workspace at `workspaces/mingai/todos/active/26-bedrock-provider.md`. Added: BEDROCK-018 TenantLLMConfig.tsx label, BEDROCK-019 LibraryModeTab.tsx embed note, BEDROCK-020 api_version payload+form-state clear, BEDROCK-021 useLLMProviders.ts ProviderType verification. Red-team confirmed ModelBreakdownTable/TenantCostTable/cost_summary_job are all data-driven — no changes needed.) Prior: TODO-15 BEDROCK-001–017 CREATED — AWS Bedrock 4th LLM Library provider: v050 migration, routes.py frozenset + publish gate + region validation + test harness branch, instrumented_client.py runtime adapter + embed exclusion, frontend LibraryForm/LibraryList/useLLMLibrary. 17 items. `todos/active/15-bedrock-provider.md` created.) Prior: 2026-03-21 (TODO-14 ATA-001–055 CREATED — Agent Template A2A Compliance: 55 items across Phase A (Sprint 1–2, Weeks 1–3), Phase B (Sprint 3–4, Weeks 4–6), Phase C (Sprint 5–8, Weeks 7–10). 4 CRITICAL gaps: guardrail enforcement never wired, agent_access_control never populated on deploy, KB bindings silently dropped, silent discard comment in routes.py. Alembic chain: v045→v046a→v046b→v047→v048. 9 new integration test files. 6 COC rules to encode in source. `todos/active/14-agent-template-a2a.md` created.) Prior: TODO-13A/B/C/D ALL COMPLETE — 13A: adaptive polling 15s/60s, Refresh button, Running badge, row pulse. 13B: GET /api/v1/tenant/jobs endpoint + TenantJobHistoryCard on /admin/sync (7 backend tests, 3 frontend tests). 13C: POST /api/v1/platform/jobs/{job_name}/trigger endpoint + RunNowButton component (6 backend tests, 3 frontend tests). 13D: run_document_sync_scheduler asyncio loop SCHED-039, main.py lifespan wiring, next_run_at in SharePoint response, "Next scheduled sync" row in SchedulerSignalsCard; backoff added to \_advance_next_run_at. 13 total backend unit tests passing, 0 TypeScript errors. Reviewer must-fix items addressed. All 4 completed todos moved to todos/completed/. active/ contains only this master index. Prior: TODO-13D added — Document Sync Scheduler. TODO-13 A/B/C created — Scheduler Monitoring Improvements. Session 41 — SCHED-001 through SCHED-038 ALL COMPLETE. Commit 6e6237d. 12-distributed-job-scheduling.md moved to todos/completed/. active/ contains only this master index. Session 40b — Red team fixes applied to 12-distributed-job-scheduling.md: 3 new items added (SCHED-036 zombie cleanup unit tests, SCHED-037 startup AOF warning, SCHED-038 per-tenant query_warming throttle). Total: 35 → 38 items. BLOCK fixes: SCHED-006 depends on SCHED-007; SCHED-023 signature corrected to no db param, own session via async_session_factory, CancelledError shield on fresh connection, yielded dataclass not dict; SCHED-004 4th index corrected to CONCURRENTLY partial index on started_at. GAP fixes: SCHED-019 moved to Phase 3; SCHED-024 depends on SCHED-019 with all-13 note; SCHED-002 self-termination via asyncio.current_task(); SCHED-031 gains 10th test case. MINOR fixes: SCHED-010–016 Phase 3 addendum for check_missed_job; pytest full paths corrected.) Session 40 — SCHED-001 through SCHED-035 created: distributed job scheduling plan. 12-distributed-job-scheduling.md added to active/. 35 items across 4 phases + testing. Fixes 2 CRITICAL (C-01 heartbeat, C-02 AOF check) + 7 HIGH red team findings including H-07 — tool_health_scheduler never wired in main.py. Alembic v042 job_run_log migration. APScheduler removal. Plan reference: 13-distributed-job-scheduling-plan.md v1.1.) Session 39 — Full intermediate review pass on all completed todos. ENTRA-008 gap found and fixed: SSO wizard re-configure mode implemented (existingEntraConfig prop, PATCH flow, pre-filled fields, client_id read-only, Azure Portal setup instructions with NEXT_PUBLIC_AUTH0_DOMAIN). AdminSSOConfig interface extended with domain/client_id optional fields. All review findings resolved. 2560 unit tests passing. 0 TypeScript errors.) **Previous**: 2026-03-17 (Todo cleanup — 05-tenant-admin-phase-b-d.md moved to todos/completed/ (35/36 COMPLETE; TA-025 product-gated, no codegen work remains). Tombstone files 06/07/08 removed from active/. active/ now contains only 00-master-index.md. All actionable Phase 2+ work is COMPLETE. 2538 unit tests passing. Gated items: TA-025 (5-10 persona interviews), DEF-015 (Auth0 CI secrets), TEST-004/005 (Auth0 CI secrets), HAR-013–017 (external: 100+ transactions / blockchain). Session 38 — PVDR-001 through PVDR-020 all COMPLETE: LLM Provider Credentials Management — DB-backed provider table (v039_llm_providers.py), ProviderService with Fernet encryption, 8-endpoint Platform API, InstrumentedLLMClient DB-resolution + 8 bypass module consolidation, embed() path migration, bootstrap auto-seed, health check job (APScheduler 600s), tenant provider selection API + UI, Anthropic embedding fallback, health summary endpoint, env var graceful fallback, platform provider list/form/banner frontend (Next.js), tenant LLM config UI extension, 9 unit tests + 11 integration tests + 8 security tests + 4 E2E scenarios. 2538 unit tests passing.) Session 39 — Full intermediate review pass on all completed todos. ENTRA-008 gap found and fixed: SSO wizard re-configure mode implemented (existingEntraConfig prop, PATCH flow, pre-filled fields, client_id read-only, Azure Portal setup instructions with NEXT_PUBLIC_AUTH0_DOMAIN). AdminSSOConfig interface extended with domain/client_id optional fields. All review findings resolved. 2560 unit tests passing. 0 TypeScript errors. Session 37 — DEF-008 COMPLETE: Auth0 group sync DB writes + login hook — `sync_roles_on_login()` calls `sync_team_memberships_db()`, upsert pattern, `test_group_sync.py` + `test_jit_provisioning.py`. DEF-014 COMPLETE: `tests/integration/test_sso_wizards.py` 6/6 SSO integration tests passing (TEST-026/027), Auth0 Management API mocked. DEF count now 17/20. 2535 total tests passing (2529 unit + 6 SSO integration). DEF-015 remains TODO — gated on Auth0 CI secrets.) Session 36 — P3AUTH-002 through P3AUTH-021 all COMPLETED except P3AUTH-001 (in progress - 4/6 criteria). P3AUTH count: 20/21 COMPLETE. TA-001/002/003/004/005/033/035 all COMPLETED. TA count: 35/36 COMPLETE (TA-025 blocked/gated). DEF-008 IN PROGRESS (parallel session). 2511 unit tests passing. Todo cleanup: 01-phase2-llm-library.md and 04-platform-admin-phase-b-d.md moved to completed/ after intermediate-reviewer verification. Active tombstone for 03-caching-c2-c4.md deleted. PA-001/003 acceptance criteria checkboxes corrected to [x]; PA-024/025 status lines fixed to COMPLETED.) Session 35 — P3AUTH-015 marked COMPLETED in 02-phase3-auth-flexibility.md: GroupSyncConfigPanel.tsx + useGroupSyncConfig/useUpdateGroupSyncConfig in useSSO.ts, 18 unit tests, committed bdc28b5. DEF-018 confirmed COMPLETED in 07-deferred-phase1.md: tests/e2e/test_memory_e2e.py 13 E2E tests, WorkingMemoryService bug fix. P3AUTH now 2/21 COMPLETE; DEF now 15/20 COMPLETE. No unblocked work remains — all remaining TODOs are Auth0-gated.) Session 34 — P2LLM-020 marked COMPLETED in 01-phase2-llm-library.md: src/web/tests/e2e/test_llm_library.spec.ts — 8 Playwright E2E tests all passing, covering Draft creation, Draft→Published lifecycle, test harness, tenant library listing, BYOLLM key not exposed, library mode selection, non-enterprise BYOLLM CTA, published count growth. 01-phase2-llm-library.md is now 20/20 COMPLETE. P3AUTH-010 marked COMPLETED in 02-phase3-auth-flexibility.md: GET/PATCH /admin/sso/group-sync/config added to app/modules/admin/workspace.py, roles validated against {admin, editor, viewer, user} allowlist, config stored in tenant_configs.config_type='sso_group_sync', 7 unit tests added — 2341 total unit tests passing.) Session 32 — HAR-001 through HAR-012 all marked COMPLETED in 06-agent-registry.md. HAR Phase 0-1: 12/12 COMPLETE. HAR-013–017 remain GATED. 2334 unit tests passing.) Session 20 — Phase 2+ todo files created. Phase 1 COMPLETE — all Phase 1 todos moved to completed/. 1134 unit tests passing as of last commit ea7481e.
...
**Phase 2+ total**: 167 new work items (177 including 8 gated and 2 blocked) across 7 todo files — all 7 now in `todos/completed/`. active/ contains only this master index. All actionable codegen work is COMPLETE as of 2026-03-17. 2538 unit tests passing.
**Phase 5 — Distributed Job Scheduling**: 38 items (SCHED-001–038) — ALL COMPLETE 2026-03-20. `todos/completed/12-distributed-job-scheduling.md`. Commit 6e6237d. DistributedJobLock (Redis SET NX EX + heartbeat + Lua), v042 job_run_log migration, all 13 jobs wrapped, APScheduler removed, H-07 permanently fixed (tool_health_scheduler now wired), job history API + Platform Admin UI, Tenant Admin sync-status endpoint, Redis AOF CI check, K8s grace period. All red team findings resolved (2 CRITICAL + 7 HIGH + 4 MEDIUM + 2 LOW).
**Phase 5 — Scheduler UX + Automation (TODO-13A–D)**: 19 items — ALL COMPLETE 2026-03-20. `todos/completed/13a–13d-*.md`. Adaptive polling (15s/60s), Refresh button, Running badge, row pulse (13A). GET /api/v1/tenant/jobs + TenantJobHistoryCard (13B, 7 tests). POST /api/v1/platform/jobs/{job_name}/trigger + RunNowButton (13C, 6 tests). run_document_sync_scheduler asyncio loop SCHED-039 + main.py lifespan + next_run_at in SharePoint response + "Next scheduled sync" UI row (13D). 13 total backend unit tests passing. 0 TypeScript errors.
**Phase 3 — pgvector migration**: 33 items (PGV-001–033) — ALL COMPLETE 2026-03-18. `todos/completed/10-pgvector-migration.md`. v040/v041 migrations applied (alembic head = 041), halfvec(1536), hybrid RRF search, per-tenant partial HNSW, Azure Search fully removed, 33 new tests passing.
...
**LLM Profile Redesign (TODO-27–34, 38)**: 9 todos across Phase A–E — ALL COMPLETE 2026-03-22. v050 schema migration (multi-slot profiles, BYOLLM track, history + audit tables), SSRF validation middleware (domain allowlist + RFC 1918 denylist + DNS rebinding protection), LLMProfileService (atomic default swap, audit trail, cache invalidation), three-tier ProfileResolver (LRU + Redis + DB, feature flag, fail-closed), slot-aware InstrumentedLLMClient (traffic splitting, SSRF re-validation at call time), Platform Profile CRUD API + plan tier middleware, Tenant Admin llm-config rewrite + BYOLLM Enterprise API, 15 integration test files (security + resolution + integrity), 24h health monitoring background job with degraded notifications. 3246 unit tests passing. All 9 todo files moved to `todos/completed/`.

---

## 1. Active Todo Files

**Last corrected**: 2026-03-22 — TODOs 13–26 restored to `active/` (were incorrectly moved to `completed/` before implementation).

| File                                                        | Description                                                                                 | Phase      |
| ----------------------------------------------------------- | ------------------------------------------------------------------------------------------- | ---------- |
| `todos/active/13-agent-studio-phase1-foundation.md`         | DB schema foundation: tools, skills, agent_template extensions, versions table, seed data   | Phase 1    |
| `todos/active/14-agent-studio-phase1-ta-agent-catalog.md`   | TA Agent Catalog UI — browse/deploy template agents, agent library page                     | Phase 1    |
| `todos/active/15-agent-studio-phase1-ta-deploy-wizard.md`   | TA Deploy Wizard — 4-step modal (template select, KB/tools, access, credentials)            | Phase 1    |
| `todos/active/16-agent-studio-phase1-ta-skills.md`          | TA Skills Library — adopt/manage platform skills per tenant                                 | Phase 1    |
| `todos/active/17-agent-studio-phase1-ta-mcp-tools.md`       | TA MCP Tools — register and test MCP server endpoints                                       | Phase 1    |
| `todos/active/18-agent-studio-phase1-ta-custom-agent.md`    | TA Custom Agent Studio — create/test/publish custom agents                                  | Phase 1    |
| `todos/active/19-agent-studio-phase1-ta-a2a.md`             | TA A2A Registry — register and test external A2A agents                                     | Phase 1    |
| `todos/active/20-agent-studio-phase2-pa-template-studio.md` | PA Template Studio — TemplateStudioPanel (800px, 4 tabs, 7 sections, SystemPromptValidator) | Phase 2    |
| `todos/active/21-agent-studio-phase2-pa-skills-library.md`  | PA Skills Library — author/publish platform skills with versioning                          | Phase 2    |
| `todos/active/22-agent-studio-phase2-pa-mcp-builder.md`     | PA MCP Builder — create and publish MCP tool definitions                                    | Phase 2    |
| `todos/active/23-agent-studio-phase2-pa-tool-catalog.md`    | PA Tool Catalog — manage built-in and HTTP-wrapper tools                                    | Phase 2    |
| `todos/active/24-agent-studio-phase2-pa-a2a-registry.md`    | PA A2A Registry — register, verify and publish external A2A agents                          | Phase 2    |
| `todos/active/25-agent-studio-phase3-advanced.md`           | Phase 3 advanced: mandatory skills enforcement, orchestrator routing, performance dashboard | Phase 3    |
| `todos/active/26-bedrock-provider.md`                       | AWS Bedrock as 4th LLM Library provider (BEDROCK-001–021)                                   | Standalone |

**Gated / deferred items** (require human decision before codegen resumes):

- **ATA-057** (remove 422 gate): Gate — Phase A (ATA-006, ATA-008, ATA-009) must be deployed to staging and verified. See `todos/completed/14-agent-template-a2a.md`.
- **Plan 08 Phase 2**: Agent-scoped memory port. Gate — customer validation of memory feature + Phase 1 complete. See `workspaces/mingai/02-plans/08-profile-memory-plan.md`.

## 1a. Recently Completed

| File                                                      | Description                                                                                                     | Items                                             | Status                        |
| --------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- | ----------------------------- |
| `todos/completed/LLM-QG-5-security-code-review-round2.md` | Security + code review agents: validate round-2 BYOLLM fixes (7 findings: 2 CRITICAL, 2 HIGH, 2 MEDIUM, 1 IPv6) | Quality gate — all findings resolved              | COMPLETE 2026-03-22           |
| `todos/completed/38-llm-profile-health-monitoring.md`     | LLM Profile Phase E: 24h background health check job, degraded notifications, UI warning indicators             | 1 job + 3 test files                              | COMPLETE 2026-03-22           |
| `todos/completed/34-llm-profile-backend-tests.md`         | LLM Profile Phase B Tests: security, resolution cache tiers, profile integrity (15 integration test files)      | 15 test files                                     | COMPLETE 2026-03-22           |
| `todos/completed/33-llm-profile-tenant-byollm-api.md`     | LLM Profile Phase B6+B7: Tenant Admin llm-config rewrite (read-only) + BYOLLM Enterprise API                    | 3 route files                                     | COMPLETE 2026-03-22           |
| `todos/completed/32-llm-profile-platform-api.md`          | LLM Profile Phase B5: Platform Profile CRUD API + plan tier middleware (12 endpoints)                           | 2 route files                                     | COMPLETE 2026-03-22           |
| `todos/completed/31-llm-profile-instrumented-client.md`   | LLM Profile Phase B4: Slot-aware InstrumentedLLMClient, traffic splitting, SSRF re-validation at call time      | 1 core module                                     | COMPLETE 2026-03-22           |
| `todos/completed/30-llm-profile-resolver.md`              | LLM Profile Phase B3: Three-tier ProfileResolver (LRU + Redis + DB), feature flag, fail-closed design           | 1 core module                                     | COMPLETE 2026-03-22           |
| `todos/completed/29-llm-profile-service.md`               | LLM Profile Phase B2: LLMProfileService — all CRUD, atomic default swap, audit trail, cache invalidation        | 1 service module                                  | COMPLETE 2026-03-22           |
| `todos/completed/28-llm-profile-ssrf-middleware.md`       | LLM Profile Phase B1: SSRF validation middleware — domain allowlist, RFC 1918 denylist, DNS rebinding check     | 1 security module                                 | COMPLETE 2026-03-22           |
| `todos/completed/27-llm-profile-schema.md`                | LLM Profile Phase A: v050 schema migration — multi-slot profiles, BYOLLM track, history + audit tables          | v050 migration                                    | COMPLETE 2026-03-22           |
| `todos/completed/14-agent-template-a2a.md`                | Agent Template A2A Compliance — schema, KB bindings, access control, guardrails, tool ecosystem                 | ATA-001–058 (57/58 complete, ATA-057 human-gated) | COMPLETE 2026-03-21 (229f3b1) |
| `todos/completed/13d-document-sync-scheduler.md`          | Document sync scheduler — asyncio loop (SCHED-039) fires SharePoint/GDrive syncs on schedule                    | D-1–D-4 (4 items)                                 | COMPLETE 2026-03-20           |
| `todos/completed/13c-platform-job-trigger.md`             | Platform Admin — manual one-shot job trigger endpoint + Run Now UI                                              | C-1–C-6 (6 items)                                 | COMPLETE 2026-03-20           |
| `todos/completed/13b-tenant-job-history.md`               | Tenant Admin — per-tenant job history endpoint + TenantJobHistoryCard frontend                                  | B-1–B-5 (5 items)                                 | COMPLETE 2026-03-20           |
| `todos/completed/13a-platform-scheduler-ux.md`            | Platform Admin — adaptive polling (15s/60s), Refresh button, Running badge, row pulse                           | A-1–A-4 (4 items)                                 | COMPLETE 2026-03-20           |
| `todos/completed/12-distributed-job-scheduling.md`        | Distributed Redis-lock job scheduling — replace APScheduler, wrap all 13 jobs                                   | 38 (SCHED-001–038)                                | COMPLETE 2026-03-20 (6e6237d) |
...
| File                                             | Description                                                      | Items               | Status                                                                                                                                                                 |
| ------------------------------------------------ | ---------------------------------------------------------------- | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `todos/completed/01-phase2-llm-library.md`       | LLM abstraction layer, multi-provider support, cost tracking     | 20 (P2LLM-001–020)  | ✅ ALL 20 COMPLETED — moved to completed/ 2026-03-17 (verified by intermediate-reviewer)                                                                               |
| `todos/completed/02-phase3-auth-flexibility.md`  | Auth0 SSO, SAML/OIDC wizards, JIT provisioning, group sync       | 21 (P3AUTH-001–021) | ✅ ALL 21 COMPLETED — P3AUTH-001 completed 2026-03-17: mingai-entra (con_gZzXoNvQ58MWRJMa), agenticOS2 Azure AD app, GroupMember.Read.All granted. Moved to completed/ |
...
| `todos/completed/04-platform-admin-phase-b-d.md` | PA health scoring, cost monitoring, templates, tool catalog      | 36 (PA-001–036)     | ✅ ALL 36 COMPLETED — moved to completed/ 2026-03-17 (verified by intermediate-reviewer; PA-001/003 checkboxes fixed, PA-024/025 status fixed)                         |
| `todos/completed/05-tenant-admin-phase-b-d.md`   | SSO wiring, access control, agent workspace, mobile              | 36 (TA-001–036)     | ✅ 35/36 COMPLETED — moved to completed/ 2026-03-17. TA-025 PRODUCT-GATED (5-10 persona interviews required). No actionable codegen work remains.                      |
| `todos/completed/06-agent-registry.md`           | HAR Phase 0-3: registry catalog, A2A gaps, KYB, blockchain       | 17 (HAR-001–017)    | ✅ 12/12 Phase 0-1 COMPLETED — moved to completed/ 2026-03-17. HAR-013–017 moved to `todos/deferred/01-blockchain-open-registry.md` (long-term deferred 2026-03-17).   |
...
| File                                            | Items       | Resume Condition                                                                  |
| ----------------------------------------------- | ----------- | --------------------------------------------------------------------------------- |
| `todos/deferred/01-blockchain-open-registry.md` | HAR-013–017 | 100+ completed transactions + product decision + customer contractual requirement |

### Deferred Item Quick Pickup Guide

See `todos/completed/07-deferred-phase1.md` section "Quick Pickup Guide" for which DEF items to pick up alongside each Phase 2 implementation task.
...
| v042    | `job_run_log` table — distributed job execution history (composite indexes H-03; `abandoned` status H-04)             | SCHED-004  | v041 (pgvector)            |
...
| ------------------------------------------ | --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `todos/completed/01-phase2-llm-library.md` | P2LLM-001 through P2LLM-019 | 19    | 26 unit tests, 5/5 BYOLLM security tests, 5/5 cost tracking tests, 5/5 cache TTL tests. Migrations v009+v010 applied. All frontend components 0 TS errors.                   |
...
**DEF status after Session 22 (updated through Session 37)**: 17/20 COMPLETE. DEF-006 and DEF-020 are gated (Phase 5 / Phase 6). DEF-015 remains TODO — gated on real Auth0 CI secrets (AUTH0*TEST*\* env vars not available in CI).

---

## 0d. Session 36 Completed Items (2026-03-17)

| Item       | Description                             | File                                            | Evidence                                                                                                                                                                             |
| ---------- | --------------------------------------- | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| P3AUTH-002 | Auth0 JWKS validation                   | `todos/completed/02-phase3-auth-flexibility.md` | `app/modules/auth/jwt.py` — decode_jwt_token_auth0(), \_JWKS_CACHE 3600s TTL, key rotation auto-invalidation. 25 unit + 5 integration tests.                                         |
| P3AUTH-003 | Tenant SSO configuration API            | `todos/completed/02-phase3-auth-flexibility.md` | GET/PATCH `/admin/sso/config` in workspace.py. provider_type allowlist. Audit log on PATCH. 20 unit tests.                                                                           |
| P3AUTH-004 | SAML 2.0 SSO wizard API                 | `todos/completed/02-phase3-auth-flexibility.md` | `app/modules/admin/sso_saml.py` — configure/sp-metadata/test endpoints. defusedxml + lxml. 22 unit tests.                                                                            |
| P3AUTH-005 | OIDC SSO wizard API                     | `todos/completed/02-phase3-auth-flexibility.md` | `app/modules/admin/sso_oidc.py` — OIDC discovery, vault-ref client secret. 14 unit tests.                                                                                            |
| P3AUTH-006 | Google Workspace SSO                    | `todos/completed/02-phase3-auth-flexibility.md` | `app/modules/admin/sso_oidc.py` — google-oauth2 strategy. Client secret encrypted.                                                                                                   |
| P3AUTH-007 | Okta SSO                                | `todos/completed/02-phase3-auth-flexibility.md` | `app/modules/admin/sso_oidc.py` — OIDC discovery reuse for Okta domain.                                                                                                              |
| P3AUTH-008 | JIT provisioning on first SSO login     | `todos/completed/02-phase3-auth-flexibility.md` | `app/modules/auth/jit_provisioning.py` — POST /internal/users/jit-provision. hmac.compare_digest. scripts/auth0/post_login_action.js. 7 unit tests.                                  |
| P3AUTH-009 | Group-to-role sync                      | `todos/completed/02-phase3-auth-flexibility.md` | `app/modules/auth/jit_provisioning.py` — POST /internal/users/sync-roles. Highest-privilege role wins. Audit log on change. 2511 unit tests passing.                                 |
| P3AUTH-011 | Session management                      | `todos/completed/02-phase3-auth-flexibility.md` | `app/modules/auth/routes.py` — Redis sorted set sessions, plan limits (starter=3/pro=10/enterprise=unlimited), force-logout endpoint.                                                |
| P3AUTH-012 | Migration tooling                       | `todos/completed/02-phase3-auth-flexibility.md` | `scripts/migrate_users_to_auth0.py` — idempotent CLI, dry-run, CSV report, 100ms rate limit.                                                                                         |
| P3AUTH-013 | Local auth fallback                     | `todos/completed/02-phase3-auth-flexibility.md` | `app/core/dependencies.py` routes HS256 → decode_jwt_token_v1_compat(). Integration test confirms local login with AUTH0_DOMAIN present.                                             |
| P3AUTH-014 | SSO configuration PATCH route wired     | `todos/completed/02-phase3-auth-flexibility.md` | `src/web/lib/hooks/useSSO.ts` — 7 new hooks. SSOSetupWizard.tsx fully wired. SP metadata download. 0 TS errors.                                                                      |
| P3AUTH-016 | Auth0 JWKS integration tests            | `todos/completed/02-phase3-auth-flexibility.md` | `tests/integration/test_auth0_jwks.py` — 5 tests, graceful skip when AUTH0_DOMAIN absent.                                                                                            |
| P3AUTH-017 | JWT v1/v2 dual acceptance int. tests    | `todos/completed/02-phase3-auth-flexibility.md` | `tests/integration/test_jwt_dual_acceptance.py` — 5 tests: v1/v2 HS256+RS256, structure validation, expiry rejection.                                                                |
| P3AUTH-018 | SAML SSO unit tests                     | `todos/completed/02-phase3-auth-flexibility.md` | 22 tests in `tests/unit/test_saml_wizard.py` — delivered as part of P3AUTH-004.                                                                                                      |
| P3AUTH-019 | OIDC SSO unit tests                     | `todos/completed/02-phase3-auth-flexibility.md` | 14 tests in `tests/unit/test_oidc_wizard.py` — delivered as part of P3AUTH-005/006/007.                                                                                              |
| P3AUTH-020 | JIT provisioning unit tests             | `todos/completed/02-phase3-auth-flexibility.md` | `tests/unit/test_jit_provisioning.py` — 7 tests (P3AUTH-008) expanded to 14 with group sync (P3AUTH-009).                                                                            |
| P3AUTH-021 | Auth0 Management API client creds       | `todos/completed/02-phase3-auth-flexibility.md` | `app/modules/auth/management_api.py` — get_management_api_token() Redis cache TTL=expires_in-60. 17 unit tests. AUTH0_MANAGEMENT_API_TOKEN deprecated.                               |
| TA-001     | SAML wizard backend API                 | `todos/completed/05-tenant-admin-phase-b-d.md`  | P3AUTH-004 endpoints in sso_saml.py under /admin/sso/saml/ with require_tenant_admin. SP metadata entityID verified.                                                                 |
| TA-002     | OIDC wizard backend API                 | `todos/completed/05-tenant-admin-phase-b-d.md`  | P3AUTH-005 endpoints in sso_oidc.py under /admin/sso/oidc/ with require_tenant_admin. Auth0 authorize URL on test.                                                                   |
| TA-003     | JIT provisioning                        | `todos/completed/05-tenant-admin-phase-b-d.md`  | GET /admin/sso/config returns jit_provisioning block. PATCH accepts jit_default_role. 6 unit tests (TestSSOConfigJITExtension).                                                      |
| TA-004     | Group-to-role mapping UI                | `todos/completed/05-tenant-admin-phase-b-d.md`  | `src/web/app/settings/sso/elements/GroupRoleMappingTable.tsx` — Add/Delete rows, role dropdown, Save wired to useUpdateGroupSyncConfig(). 0 TS errors.                               |
| TA-005     | SSO enable/disable toggle               | `todos/completed/05-tenant-admin-phase-b-d.md`  | `src/web/app/settings/sso/elements/SSOToggle.tsx` — 3 render states, confirm dialog on disable. 0 TS errors.                                                                         |
| TA-033     | User import from SSO                    | `todos/completed/05-tenant-admin-phase-b-d.md`  | `app/modules/admin/sso_import.py` — directory/users + import-from-sso. Migration v037 users.auth0_user_id. 26 unit tests.                                                            |
| TA-035     | Tenant admin role delegation            | `todos/completed/05-tenant-admin-phase-b-d.md`  | `app/modules/admin/role_delegation.py` — POST/DELETE/GET delegation. Migration v038 user_delegations + RLS. 21 unit tests.                                                           |
| DEF-008    | Auth0 group sync DB writes + login hook | `todos/completed/07-deferred-phase1.md`         | ✅ COMPLETE — `sync_roles_on_login()` calls `sync_team_memberships_db()` on every login. Upsert pattern. `test_group_sync.py` + `test_jit_provisioning.py`. 2529 unit tests passing. |
| DEF-014    | SAML/OIDC SSO integration tests         | `todos/completed/07-deferred-phase1.md`         | ✅ COMPLETE — `tests/integration/test_sso_wizards.py` — 6/6 passing (TEST-026/027). Real PostgreSQL, Auth0 Management API mocked. `_run_sync()` KQueue fix.                          |

**P3AUTH status after Session 36**: 20/21 COMPLETE. P3AUTH-001 is the only remaining item (IN PROGRESS — 4/6 acceptance criteria met: Auth0 tenant created, .env.example updated, app + API resource created, local .env populated; remaining: Azure Entra upstream connection + SSO connection ID documentation).

**TA status after Session 36**: 35/36 COMPLETE. TA-025 (Agent Studio) remains BLOCKED on product gate (5-10 persona interviews). All other TA items are COMPLETE.

**DEF status after Session 36**: 15/20 COMPLETE. DEF-008 IN PROGRESS. DEF-006/020 GATED. DEF-014/015 remain TODO (integration/E2E tests for SSO wizards — actionable now that P3AUTH-004/005 are complete).

---

## 0d-a. Session 37 Completed Items (2026-03-17)

| Item    | Description                             | File                                    | Evidence                                                                                                                                                                                                                                                                                                            |
| ------- | --------------------------------------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| DEF-008 | Auth0 group sync DB writes + login hook | `todos/completed/07-deferred-phase1.md` | `app/modules/auth/jit_provisioning.py` — `sync_roles_on_login()` calls `sync_team_memberships_db()` on every login. `app/modules/auth/group_sync.py` — writes/removes `team_memberships` rows via INSERT ON CONFLICT upsert. `tests/unit/test_group_sync.py` + `test_jit_provisioning.py`. 2529 unit tests passing. |
| DEF-014 | SAML/OIDC SSO integration tests         | `todos/completed/07-deferred-phase1.md` | `tests/integration/test_sso_wizards.py` — 6 integration tests (TEST-026/027): SAML configure from XML + URL, SP metadata download, 409 duplicate, OIDC configure with discovery, OIDC test URL. Real PostgreSQL, Auth0 Management API mocked. `_run_sync()` helper for macOS KQueue conflicts. 6/6 passing.         |

**DEF status after Session 37**: 17/20 COMPLETE. DEF-006/020 GATED. DEF-015 remains TODO — gated on real Auth0 CI secrets (AUTH0*TEST*\* env vars not available). 2535 total tests passing (2529 unit + 6 SSO integration).
